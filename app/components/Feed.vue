...
<template>
  <UPageGrid class="gap-0.5 grid-cols-3 sm:grid-cols-3 lg:grid-cols-3 ">
    <UPageCard
v-for="(card, index) in global.feed" :key="index" class="rounded-none" v-bind="card"
      :ui="{ container: card.image && 'p-0! gap-y-0 gap-x-0', wrapper: 'absolute right-0', leading: 'p-2 sm:p-4', leadingIcon: 'text-white' }">
      <template #default>
        <component :is="getComponentById(card.id)" v-if="card.id && getComponentById(card.id)" />
        <UModal v-else-if="card.image" fullscreen>
          <UButton color="neutral" variant="subtle" class="size-full" :ui="{ base: 'p-0' }">
            <NuxtPicture
:src="card.image" format="webp" :img-attrs="{ class: 'size-full object-cover' }"
              :alt="`Image ${index + 1}`" class="w-full h-full" loading="lazy" />
          </UButton>
          <template #body>
            <NuxtPicture
:src="card.image" format="webp" :img-attrs="{ class: 'size-full object-contain' }"
              :alt="`Image ${index + 1}`" class="w-full h-full" loading="lazy" />
          </template>
        </UModal>
      </template>
