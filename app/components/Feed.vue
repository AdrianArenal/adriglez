...
<template>
  <UPageGrid class="gap-0.5 grid-cols-3 sm:grid-cols-3 lg:grid-cols-3 ">
    <UPageCard class="rounded-none" v-for="(card, index) in global.feed" :key="index" v-bind="card"
      ...
      <template v-if="card.image" #default>
        <UModal fullscreen>
          <UButton color="neutral" variant="subtle" class="size-full" :ui="{ base: 'p-0' }">
            <NuxtPicture :src="card.image" format="webp" :img-attrs="{ class: 'size-full object-cover' }"
              :alt="`Image ${index + 1}`" class="w-full h-full" loading="lazy" />
          </UButton>
          <template #body>
            <NuxtPicture :src="card.image" format="webp" :img-attrs="{ class: 'size-full object-contain' }"
              :alt="`Image ${index + 1}`" class="w-full h-full" loading="lazy" />
          </template>
        </UModal>
      </template>
